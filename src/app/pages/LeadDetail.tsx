...
export default function LeadDetail() {
  ...
  const getStatusBadgeColor = (status: string) => {
    if (['Auto-Contacted', 'Engaged', 'Follow-up'].some(s => status.includes(s))) return 'bg-blue-100 text-blue-700';
    if (['New Lead', 'No Response'].some(s => status.includes(s))) return 'bg-yellow-100 text-yellow-700';
    if (['Scheduling', 'Negotiation', 'Nurture', 'Site Visit'].some(s => status.includes(s))) return 'bg-purple-100 text-purple-700';
    if (['Site Visit Scheduled', 'Site Visit Completed', 'Closed Won'].some(s => status.includes(s))) return 'bg-green-100 text-green-700';
    if (['Closed Lost', 'Not Interested', 'Invalid'].some(s => status.includes(s))) return 'bg-red-100 text-red-700';
            ...
            <Button
              ...
              onClick={() => {
                ...
                const colorMap: Record<string, string> = {
                  'Auto-Contacted': 'bg-blue-500',
                  'Engaged': 'bg-blue-500',
                  'Follow-up': 'bg-blue-500',
                  'New Lead': 'bg-yellow-500',
                  'No Response': 'bg-yellow-500',
                  'Scheduling': 'bg-purple-500',
                  'Negotiation': 'bg-purple-500',
                  'Nurture': 'bg-purple-500',
                  'Site Visit Scheduled': 'bg-green-500',
                  'Site Visit Completed': 'bg-green-500',
